...
@router.post("/create")
async def create_history_context(
    request: CreateHistoryContextRequest,
    ...
    try:
        ...
        print(f"[HISTORY_CONTEXT] 🔐 Authenticating user...")
        user_response = await get_user_from_token(token)
        if not user_response["success"]:
            error_message = user_response.get("error", "Authentication required")
            print(f"[HISTORY_CONTEXT] ❌ Authentication failed: {error_message}")
        ...
        if user_role not in ["admin", "teacher"]:
            print(f"[HISTORY_CONTEXT] ❌ Access denied: User role '{user_role}' is not authorized")
            raise HTTPException(status_code=403, detail="Only teachers and admins can create history context")
            
        print(f"[HISTORY_CONTEXT] ✅ User authenticated successfully. User ID: {user_id}, Role: {user_role}")
    except HTTPException as auth_error:
        print(f"[HISTORY_CONTEXT] ❌ HTTP exception during authentication: {str(auth_error)}")
        raise auth_error
    except Exception as auth_error:
        print(f"[HISTORY_CONTEXT] ❌ Unexpected error during authentication: {str(auth_error)}")
    ...
    try:
        ...
        prompt = load_prompt("prompts/gen_case_smry_for_hist_v3.txt")
