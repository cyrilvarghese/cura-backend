...
class CreateTreatmentContextRequest(BaseModel):
    file_name: str
    case_id: Optional[int] = None

@router.post("/create")
async def create_treatment_context(
    request: CreateTreatmentContextRequest,
    credentials: HTTPAuthorizationCredentials = Depends(security)
):
    """
    Create treatment context data based on a case document.
    Extracts structured treatment information using Gemini AI.
    """
    print(f"[{datetime.now()}] Starting treatment context creation for file: {request.file_name}, case_id: {request.case_id}")
    ...
    try:
        ...
        print(f"[TREATMENT_CONTEXT] 🔐 Authenticating user...")
        user_response = await get_user_from_token(token)
        if not user_response["success"]:
            error_message = user_response.get("error", "Authentication required")
            print(f"[TREATMENT_CONTEXT] ❌ Authentication failed: {error_message}")
        ...
        if user_role not in ["admin", "teacher"]:
            print(f"[TREATMENT_CONTEXT] ❌ Access denied: User role '{user_role}' is not authorized")
            raise HTTPException(status_code=403, detail="Only teachers and admins can create treatment context")
            
        print(f"[TREATMENT_CONTEXT] ✅ User authenticated successfully. User ID: {user_id}, Role: {user_role}")
    except HTTPException as auth_error:
        print(f"[TREATMENT_CONTEXT] ❌ HTTP exception during authentication: {str(auth_error)}")
        raise auth_error
    except Exception as auth_error:
        print(f"[TREATMENT_CONTEXT] ❌ Unexpected error during authentication: {str(auth_error)}")
    ...
    try:
        ...
        prompt = load_prompt("prompts/gen_trtmnt_context.txt")
        ...
        try:
            ...
            treatment_context = json.loads(cleaned_json)
            ...
            output_file = case_dir / "treatment_context.json"
            with open(output_file, 'w') as f:
                json.dump(treatment_context, f, indent=2)
            
            print(f"[{datetime.now()}] ✅ Saved treatment context to {output_file}")
        ...
        print(f"[{error_timestamp}] ❌ Error in create_treatment_context: {str(e)}")
